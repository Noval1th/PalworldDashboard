...
web/
  index.html                        the dashboard (static; fetches ./palworld.json) — generated by debrand.py
...
### Keeping `web/index.html` in sync

`web/index.html` is **generated**, not hand-edited. The page that actually runs is the copy on the game box;
`debrand.py` turns it into the deployment-neutral version published here:

```powershell
python debrand.py C:\path\to\your\live\palworld.html   # writes web/index.html
```

It strips the two things that are deployment details rather than features — the server's own name (the page
instead fills it in at runtime from `palworld.json`'s `info.servername`, so every deployment shows its own)
and the Discord invite (now a `DISCORD_URL` constant near the top of the script, empty by default, with the
"Join the Server" button hidden unless you set it).

It then **refuses to write** if any identifying string survives — server name, invite link, IP, hostname, or
box path. That check earns its keep: the server name appears in three places, and the third is a desktop
**notification title** buried in the connection-alerts code, which is exactly the sort of thing a hand
de-brand misses. This file previously drifted ~17 KB behind the live page for that reason. Run the script
